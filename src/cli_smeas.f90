      ...
      !! prepend slr_path to station file if set
      if (trim(in_cli%slr_cli) == "null" .or. len_trim(in_cli%slr_cli) == 0  &
          .or. trim(in_path_slr%slr) == "null" .or. len_trim(in_path_slr%slr) == 0) then
        cli_file = in_cli%slr_cli
      else
        cli_file = TRIM(ADJUSTL(in_path_slr%slr))//TRIM(in_cli%slr_cli)
      end if
