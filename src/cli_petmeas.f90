      ...
      !! prepend pet_path to station file if set
      if (trim(in_cli%pet_cli) == "null" .or. len_trim(in_cli%pet_cli) == 0  &
          .or. trim(in_path_pet%peti) == "null" .or. len_trim(in_path_pet%peti) == 0) then
        cli_file = in_cli%pet_cli
      else
        cli_file = TRIM(ADJUSTL(in_path_pet%peti))//TRIM(in_cli%pet_cli)
      end if
