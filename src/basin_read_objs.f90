...
!!     and reservoirs
      ...
      if (.not. open_file(107, in_sim%object_cnt, required=.true.)) then
          stop
      else
      do
      ...
      if(bsn_cc%gwflow == 1 ) then
        ...
        inquire(file=in_gwf%chancells,exist=i_exist)
        if(i_exist) then
          if(sp_ob%gwflow == 0) then
            open(107,file=in_gwf%chancells)
            read(107,*,iostat=eof) header
            if(eof == 0) then
            read(107,*,iostat=eof)
            read(107,*,iostat=eof) header
