       ...
       if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
        if (pco%chan_bsn%d == "y") then
          ...
          if (pco%sqliteout == "y") then
            call sqlite_insert_basin_ch("d", time%day, time%mo, time%day_mo, time%yrc, 1_8, 1, bsn%name, bch_d)
          end if
#endif
        end if 
      end if
...
#ifdef SQLITE_ENABLED
          if (pco%sqliteout == "y") then
            call sqlite_insert_basin_ch("m", time%day, time%mo, time%day_mo, time%yrc, 1_8, 1, bsn%name, bch_m)
          end if
#endif
        end if
        bch_m = chz
...
#ifdef SQLITE_ENABLED
          if (pco%sqliteout == "y") then
            call sqlite_insert_basin_ch("y", time%day, time%mo, time%day_mo, time%yrc, 1_8, 1, bsn%name, bch_y)
          end if
#endif
        end if
...
#ifdef SQLITE_ENABLED
        if (pco%sqliteout == "y") then
          call sqlite_insert_basin_ch("a", time%day, time%mo, time%day_mo, time%yrc, 1_8, 1, bsn%name, bch_a)
        end if
#endif
      end if
