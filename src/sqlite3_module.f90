      ...
      pure function fortran_to_c_string(fstring) result(cstring)
        character(len=*), intent(in) :: fstring
        character(len=len_trim(fstring)+1) :: cstring
      ...
      function sqlite3_open(filename, db) result(rc)
        ...
        rc = c_sqlite3_open(fortran_to_c_string(filename), db%ptr)
      ...
      function sqlite3_exec(db, sql) result(rc)
        ...
        rc = c_sqlite3_exec(db%ptr, fortran_to_c_string(sql), c_null_funptr, c_null_ptr, errmsg)
      ...
      function sqlite3_prepare_v2(db, sql, stmt) result(rc)
        ...
        rc = c_sqlite3_prepare_v2(db%ptr, fortran_to_c_string(sql), -1_c_int, stmt%ptr, tail)
